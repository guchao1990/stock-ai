...
class AIAnalyzer:
    ...
    def analyze(self, stock_data, indicators, financial_data, news_data, stock_code, save_path, stock_info=None):
        """
        ...
        参数:
            ...
            news_data (list): 新闻数据
            stock_code (str): 股票代码
            stock_info (dict): 股票基本信息(市值、市盈率、市净率等)
        ...
        try:
            ...
            try:
                ...
                stock_info_em = ak.stock_individual_info_em(symbol=stock_code)
                if not stock_info_em.empty:
                    stock_name = stock_info_em.loc[stock_info_em['item'] == '股票简称', 'value'].values[0]
                else:
            ...
            # 准备分析数据
            analysis_data = self._prepare_analysis_data(stock_data, indicators, financial_data, news_data, stock_code, stock_name, stock_info)
    ...
    def _prepare_analysis_data(self, stock_data, indicators, financial_data, news_data, stock_code, stock_name, stock_info=None):
        """
        准备用于分析的数据
        """
        analysis_data = {}
        ...
        # 添加市值、市盈率、市净率、换手率、成交额等指标
        if stock_info:
            analysis_data['市值指标'] = {
                '总市值': stock_info.get('total_market_cap', 'N/A'),
                '流通市值': stock_info.get('float_market_cap', 'N/A'),
                '市盈率(动态)': stock_info.get('pe_ratio', 'N/A'),
                '市净率': stock_info.get('pb_ratio', 'N/A'),
                '换手率': stock_info.get('turnover_rate', 'N/A'),
                '成交额': stock_info.get('volume_amount', 'N/A'),
                '总股本': stock_info.get('total_shares', 'N/A'),
                '流通股本': stock_info.get('float_shares', 'N/A'),
            }
